        ...
        for batch in data:
            ...
            for i in range(5):
                if "bald" in prompts[i] and kwds[i]=="hair":
                    ...
                    instruction = "This person now has a beard"
                elif "smiling" in prompts[i] and kwds[i]=="smiling":
                    instruction = "This person is not smiling anymore"
                elif "smiling" in prompts[i+1] and kwds[i]=="smiling":
                    instruction = "Make this person smile"
                elif "glasses" in prompts[i] and kwds[i]=="glasses":
                    instruction = "Take the glasses off this person's face"
                elif "glasses" in prompts[i+1] and kwds[i]=="glasses":
                    instruction = "This person now wears glasses"
                elif "dark hair" in prompts[i] and kwds[i]=="hair":
                    instruction = "This person now has light hair"
                else:
                    instruction = "This person now has dark hair"
                img = self.pipe(
                  image=img,
                  prompt=instruction,
                  guidance_scale=2.5
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path",type=str, default="FFHQ-Kontext")
